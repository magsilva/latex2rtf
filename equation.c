...
static void emit_math_char(int fontnum, int unicode, char *hex)
{
	if (g_processing_fields) 
		fprintRTF("{\\f%d\\u%d\\'%s}", fontnum, unicode, hex);
	else
...
/******************************************************************************
 purpose   : Handles characters that should be handled differently when they
             are in equations with fields than otherwise
             not used at the moment.
 ******************************************************************************/
void CmdMathChar(int code)
{
    int sym = RtfFontNumber("Symbol");
    diagnostics(1, "CmdMathChar() %d", code);

    switch (code) {
        case EQN_MATH_EQUIV:
        	emit_math_char(sym, code, "ba");
            break;
        case EQN_MATH_LEFTARROW:
        	emit_math_char(sym, code, "ac");
            break;
        case EQN_MATH_UPARROW:
        	emit_math_char(sym, code, "ad");
            break;
       case EQN_MATH_RIGHTARROW:
        	emit_math_char(sym, code, "ae");
            break;
       case EQN_MATH_DOWNARROW:
        	emit_math_char(sym, code, "af");
            break;
       case EQN_MATH_LEFTRIGHTARROW:
        	emit_math_char(sym, code, "ab");
            break;
       case EQN_MATH_UPDOWNARROW:
			fprintRTF("{\\u%d*}", (g_processing_fields)  ? code : 8597);
            break;
       case EQN_MATH_NWARROW:
			fprintRTF("{\\u%d*}", (g_processing_fields)  ? code : 8598);
            break;
       case EQN_MATH_NEARROW:
			fprintRTF("{\\u%d*}", (g_processing_fields)  ? code : 8599);
            break;
       case EQN_MATH_SEARROW:
			fprintRTF("{\\u%d*}", (g_processing_fields)  ? code : 8600);
            break;
       case EQN_MATH_SWARROW:
			fprintRTF("{\\u%d*}", (g_processing_fields)  ? code : 8601);
            break;
